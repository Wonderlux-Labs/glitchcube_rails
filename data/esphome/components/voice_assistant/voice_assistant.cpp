...
#ifdef USE_MEDIA_PLAYER
    // GLITCHCUBE PATCH for esphome/home-assistant-voice-pe#563 (self-STT loop).
    // Stock behavior treats this timeout firing as "announcement finished", but with
    // cloud TTS the announcement often takes >2s just to START (URL fetch/buffering),
    // so RESPONSE_FINISHED fired while the TTS was still coming — the mic reopened in
    // continue_conversation mode and STT transcribed the device's own speech.
    // Keep waiting while the announcement is genuinely underway (media player is
    // ANNOUNCING) or still starting up (URL_SENT), with bounded retries for the
    // starting-up case so a failed announcement can't wedge the satellite.
    if (this->media_player_ != nullptr) {
      bool announcing =
          this->media_player_->state == media_player::MediaPlayerState::MEDIA_PLAYER_STATE_ANNOUNCING;
      bool starting = this->media_player_response_state_ == MediaPlayerResponseState::URL_SENT;
      if (announcing || (starting && this->playback_start_retries_ < 15)) {
        if (announcing) {
          this->playback_start_retries_ = 0;
        } else {
          this->playback_start_retries_++;
        }
        ESP_LOGD(TAG, "Announcement still %s; deferring response finished",
                 announcing ? "playing" : "starting");
        this->start_playback_timeout_();
        return;
      }
      this->playback_start_retries_ = 0;
      ...
      if (this->media_player_ != nullptr) {
        for (const auto &arg : msg.data) {
          if ((arg.name == "tts_start_streaming") && (arg.value == "1") && !this->tts_response_url_.empty()) {
            ...
            this->playback_start_retries_ = 0;  // GLITCHCUBE PATCH (VPE #563)
