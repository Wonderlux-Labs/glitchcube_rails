...
| Semantic recall of people/preferences | **HASS-side memory** (`ha-ai-memory`, PERMEAR, Graphiti/Neo4j) | **Skip — fragments memory.** Rails already owns continuity (layered summarizer, see [`memory.md`](memory.md)) and has a dormant `Memory` + pgvector + `MemorySearchService` to revive. Keep one source of truth. |
