      ...
      def tool.validation_blocks
        @validation_blocks ||= [
          proc do |params, errors|
            effect = params.transform_keys(&:to_s)["effect"]

            if effect.blank?
              errors << "effect is required."
            elsif !EFFECTS.include?(effect)
              errors << "Unknown sound effect '#{effect}'. Available: #{EFFECTS.join(', ')}."
            end
